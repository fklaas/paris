(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .po-form{max-width:620px;margin:24px auto 0;text-align:left}.po-field{display:grid;gap:7px;margin:16px 0}.po-field label{font-weight:900;color:#405a6e}.po-field input{width:100%;box-sizing:border-box;padding:15px 17px;border:1px solid #d9e2e7;border-radius:16px;background:#fff;font:inherit;color:#2f485c;outline:none}.po-field input:focus{border-color:#e47b99;box-shadow:0 0 0 4px rgba(228,123,153,.12)}.po-actions{display:flex;justify-content:center;gap:11px;flex-wrap:wrap;margin-top:24px}.po-btn{border:0;border-radius:999px;padding:14px 23px;font-weight:950;cursor:pointer;transition:.18s}.po-btn.primary{background:linear-gradient(135deg,#e96d92,#d95780);color:#fff;box-shadow:0 11px 25px rgba(217,87,128,.26)}.po-btn.secondary{background:#edf4f6;color:#405a6e}.po-btn:hover{transform:translateY(-1px)}.po-btn:disabled{opacity:.55;cursor:wait;transform:none}.po-error{display:none;margin:14px 0 0;padding:11px 14px;border-radius:13px;background:#fdebed;color:#9e3f55;font-weight:750}.po-error.show{display:block}
  ...
  function chooseMode(onSolo, onShared, back) {
    const root = setStage(2, `
      <div class="po-slide po-centered">
        <div><span class="po-kicker">❤️ Gemeinsam oder alleine?</span><h1 class="po-title small">Wie möchtet ihr Erinnerungen sammeln?</h1><p class="po-copy">Diese Auswahl legt fest, ob nur dieses Gerät oder zwei persönliche Geräte zur gemeinsamen Paris-Reise gehören.</p></div>
        <div class="po-choice-grid">
          <button class="po-choice" data-solo><div class="po-choice-art"><span class="po-person">Ich</span></div><strong>Ich reise alleine</strong><p>Ein Profil, ein Gerät und trotzdem Cloud-Sicherung für eure Reise.</p></button>
          <button class="po-choice shared" data-shared><span class="po-choice-badge">Empfohlen für euch</span><div class="po-choice-art"><span class="po-person">F</span><span style="color:#e36e91;font-size:27px">♡</span><span class="po-person blue">L</span></div><strong>Wir reisen gemeinsam</strong><p>Fabian und Luisa erhalten je ein eigenes Profil und teilen denselben Stand.</p></button>
        </div>
  ...
  function joinShared(client, resolve, prefill = '', back) {
    const root = setStage(3, `
      <div class="po-slide po-centered"><div><span class="po-kicker">📱 Reise beitreten</span><h1 class="po-title small">Schön, dass du mitreist.</h1><p class="po-copy">Beim Scannen des QR-Codes ist der Einladungscode bereits eingesetzt.</p></div><div class="po-form"><div class="po-field"><label>Einladungscode</label><input data-code value="${prefill}" autocomplete="off" autocapitalize="characters"></div><div class="po-field"><label>Dein Name</label><input data-name value="${prefill ? 'Luisa' : ''}" autocomplete="name"></div><div class="po-error"></div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-go>Mit der Reise verbinden</button></div></div></div>`);
    root.querySelector('[data-back]').onclick = back;
    root.querySelector('[data-go]').onclick = async event => {
  ...
  function run(client, resolve) {
    const backToWelcome = () => welcome(() => chooseMode(
      ...
      () => sharedSetup(client, resolve, backToModes),
      backToWelcome
    ));
