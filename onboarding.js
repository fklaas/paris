(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .po-form{max-width:620px;margin:24px auto 0;text-align:left}.po-field{display:grid;gap:7px;margin:16px 0}.po-field label{font-weight:900;color:#405a6e}.po-field input{width:100%;box-sizing:border-box;padding:15px 17px;border:1px solid #d9e2e7;border-radius:16px;background:#fff;font:inherit;color:#2f485c;outline:none}.po-field input:focus{border-color:#e47b99;box-shadow:0 0 0 4px rgba(228,123,153,.12)}
      .po-create-progress{display:flex;justify-content:center;gap:7px;margin:4px 0 22px}.po-create-progress i{display:block;width:30px;height:7px;border-radius:99px;background:#e8eef0;transition:.25s}.po-create-progress i.done{background:#f1a0b8}.po-create-progress i.active{width:46px;background:linear-gradient(90deg,#e96d92,#8fcfd1)}
      .po-option-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:20px 0}.po-option{border:1px solid #e1e7ea;border-radius:18px;background:#fff;padding:17px 12px;color:#405a6e;font-weight:850;cursor:pointer;min-height:76px;transition:.2s;box-shadow:0 7px 18px rgba(52,72,89,.05)}.po-option:hover,.po-option:focus-visible{transform:translateY(-2px);outline:none;border-color:#e99ab0}.po-option.selected{border-color:#e76f91;background:#fff2f6;color:#b84f70;box-shadow:0 0 0 3px rgba(231,111,145,.11)}.po-option .po-option-icon{display:block;font-size:27px;margin-bottom:7px}.po-symbol-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.po-symbol-grid .po-option{font-size:28px;min-height:68px;padding:14px}.po-color-grid{display:flex;justify-content:center;gap:13px;flex-wrap:wrap;margin:22px 0}.po-color{width:48px;height:48px;border-radius:50%;border:5px solid #fff;box-shadow:0 0 0 1px #dce4e7,0 7px 17px rgba(52,72,89,.11);cursor:pointer;transition:.2s}.po-color.selected{box-shadow:0 0 0 3px #405a6e,0 8px 19px rgba(52,72,89,.17);transform:scale(1.07)}.po-create-summary{display:grid;grid-template-columns:auto 1fr;gap:18px;align-items:center;margin:22px 0;padding:20px;border:1px solid #e1e8ea;border-radius:24px;background:linear-gradient(145deg,#fff8fa,#f4fbfb);text-align:left}.po-create-symbol{display:grid;place-items:center;width:76px;height:76px;border-radius:23px;color:#fff;font-size:36px;box-shadow:0 14px 28px rgba(54,73,91,.15)}.po-summary-name{font:900 25px Georgia;color:#304b61}.po-summary-meta{margin-top:6px;color:#71818e;line-height:1.6}.po-date-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.po-helper{color:#87949d;font-size:13px;line-height:1.5;margin:7px 0 0}
      @media(max-width:620px){.po-option-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.po-symbol-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.po-date-grid{grid-template-columns:1fr}.po-create-summary{grid-template-columns:1fr;text-align:center}.po-create-symbol{margin:auto}}.po-scan-row{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:center}.po-scan-note{margin:8px 0 0;color:#7a8791;font-size:13px;line-height:1.5}.po-scanner{position:fixed;inset:0;z-index:30100;display:grid;place-items:center;padding:18px;background:rgba(26,42,54,.86);backdrop-filter:blur(8px)}.po-scanner-card{width:min(480px,100%);padding:18px;border-radius:28px;background:#fff;box-shadow:0 28px 80px rgba(0,0,0,.35);text-align:center}.po-scanner-video{width:100%;aspect-ratio:1;border-radius:20px;object-fit:cover;background:#172631}.po-scanner-guide{position:relative;margin-top:-100%;aspect-ratio:1;pointer-events:none;border:3px solid rgba(255,255,255,.9);border-radius:24px;transform:scale(.72)}.po-scanner-card h3{color:#304b61;margin:16px 0 6px}.po-scanner-card p{color:#71818e;margin:0 0 14px}.po-actions{display:flex;justify-content:center;gap:11px;flex-wrap:wrap;margin-top:24px}.po-btn{border:0;border-radius:999px;padding:14px 23px;font-weight:950;cursor:pointer;transition:.18s}.po-btn.primary{background:linear-gradient(135deg,#e96d92,#d95780);color:#fff;box-shadow:0 11px 25px rgba(217,87,128,.26)}.po-btn.secondary{background:#edf4f6;color:#405a6e}.po-btn:hover{transform:translateY(-1px)}.po-btn:disabled{opacity:.55;cursor:wait;transform:none}.po-error{display:none;margin:14px 0 0;padding:11px 14px;border-radius:13px;background:#fdebed;color:#9e3f55;font-weight:750}.po-error.show{display:block}
      ...
      .po-chooser-topbar{display:flex;justify-content:space-between;gap:10px;margin-bottom:6px}.po-icon-action{border:1px solid #e4e9ec;border-radius:999px;background:#fff;padding:10px 14px;color:#466074;font-weight:850;cursor:pointer}.po-icon-action.danger{color:#b54b63;background:#fff5f7}.po-trip-chooser{max-width:820px;text-align:left}.po-chooser-intro{text-align:center}.po-trip-created{display:block;margin-top:5px}.po-trip-actions{display:flex;gap:8px;flex-wrap:wrap}.po-btn.danger{background:#fff0f3;color:#b54b63}@media(max-width:760px){.paris-onboarding{place-items:start center}.po-shell{min-height:100dvh;border-radius:0}.po-trip-option{grid-template-columns:1fr}.po-trip-actions{display:grid;grid-template-columns:1fr}.po-trip-actions .po-btn{width:100%}}
    `;
    ...
    root.innerHTML = `
      <section class="po-shell" role="dialog" aria-modal="true" aria-label="Paris Reise einrichten">
        <header class="po-header">
          <div class="po-brand"><span class="po-brand-mark">♡</span><span>Unsere neue Reise</span></div>
  ...
  function soloSetup(client, resolve, back) {
    ...
    root.querySelector('[data-go]').onclick = async event => {
      ...
      try {
        ...
        const tripId = await rpcCreate(client, 'Meine Reise', joinCode, memberName);
  ...
  function createShared(client, resolve, back) {
    ...
    const knownTrips = parse(localStorage.getItem(REGISTRY_KEY)) || [];
    const existingProfile = forceNew ? null : (knownTrips.find(item => item?.tripId === existingTrip) || null);
    const accountName = cleanName(window.ParisAuth?.getState?.()?.user?.user_metadata?.display_name || window.ParisAuth?.getState?.()?.user?.user_metadata?.first_name || deviceIdentity()?.memberName || 'Fabian');
    const draft = {
      destination: existingProfile?.destination || '',
      tripName: existingProfile?.tripName || '',
      symbol: existingProfile?.symbol || '❤️',
      accent: existingProfile?.accent || '#e76f91',
      tripType: existingProfile?.tripType || 'couple',
      startDate: existingProfile?.startDate || '',
      endDate: existingProfile?.endDate || '',
      memberName: existingProfile?.memberName || accountName
    };
    const total = 7;
    const progress = step => `<div class="po-create-progress" aria-label="Schritt ${step} von ${total}">${Array.from({length:total},(_,i)=>`<i class="${i+1<step?'done':i+1===step?'active':''}"></i>`).join('')}</div>`;
    const safe = value => String(value || '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));

    const stepDestination = () => {
      const root = setStage(3, `<div class="po-slide po-centered"><div><span class="po-kicker">🧭 Neue Reise · 1 von ${total}</span><h1 class="po-title small">Wo soll es hingehen?</h1><p class="po-copy">Ein Ort, eine Region oder einfach die Idee für euer nächstes Abenteuer.</p></div><div class="po-form">${progress(1)}<div class="po-field"><label>Reiseziel</label><input data-value value="${safe(draft.destination)}" placeholder="z. B. Paris, Toskana oder Nordsee" autocomplete="off" autofocus></div><p class="po-helper">Du kannst das Ziel später jederzeit ergänzen oder ändern.</p><div class="po-error"></div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-back]').onclick = back;
      root.querySelector('[data-next]').onclick = () => { const v=cleanName(root.querySelector('[data-value]').value); if(!v)return showError('Bitte gib ein Reiseziel ein.'); draft.destination=v; stepName(); };
    };

    const stepName = () => {
      const suggestion = draft.tripName || `${draft.destination} · Unsere Reise`;
      const root = setStage(3, `<div class="po-slide po-centered"><div><span class="po-kicker">✍️ Neue Reise · 2 von ${total}</span><h1 class="po-title small">Wie soll eure Reise heißen?</h1><p class="po-copy">Wähle einen Namen, der sich später wie der Titel eurer gemeinsamen Geschichte anfühlt.</p></div><div class="po-form">${progress(2)}<div class="po-field"><label>Reisename</label><input data-value value="${safe(suggestion)}" placeholder="z. B. Unser Sommer in Italien" maxlength="70" autofocus></div><div class="po-error"></div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-back]').onclick = stepDestination;
      root.querySelector('[data-next]').onclick = () => { const v=String(root.querySelector('[data-value]').value||'').trim().slice(0,70); if(!v)return showError('Bitte gib deiner Reise einen Namen.'); draft.tripName=v; stepSymbol(); };
    };

    const stepSymbol = () => {
      const symbols=['❤️','✈️','🌴','📸','📍','⛰️','⭐','⛵','🚐','🌍','✨','🏕️'];
      const root=setStage(3,`<div class="po-slide po-centered"><div><span class="po-kicker">✨ Neue Reise · 3 von ${total}</span><h1 class="po-title small">Welches Symbol passt dazu?</h1><p class="po-copy">Das Symbol macht eure Reise in der Auswahl sofort wiedererkennbar.</p></div><div class="po-form">${progress(3)}<div class="po-option-grid po-symbol-grid" data-options>${symbols.map(x=>`<button type="button" class="po-option ${x===draft.symbol?'selected':''}" data-value="${x}">${x}</button>`).join('')}</div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-options]').onclick=e=>{const b=e.target.closest('[data-value]');if(!b)return;draft.symbol=b.dataset.value;root.querySelectorAll('[data-value]').forEach(x=>x.classList.toggle('selected',x===b));};
      root.querySelector('[data-back]').onclick=stepName; root.querySelector('[data-next]').onclick=stepColor;
    };

    const stepColor = () => {
      const colors=['#e76f91','#d96b61','#e39a48','#d4ad4f','#7c9b67','#4ca7a0','#5e8ccf','#836ec7'];
      const root=setStage(3,`<div class="po-slide po-centered"><div><span class="po-kicker">🎨 Neue Reise · 4 von ${total}</span><h1 class="po-title small">Welche Farbe begleitet euch?</h1><p class="po-copy">Der Farbakzent gibt Karten, Symbolen und späteren Reiseübersichten ihren eigenen Charakter.</p></div><div class="po-form">${progress(4)}<div class="po-color-grid" data-colors>${colors.map(x=>`<button type="button" class="po-color ${x===draft.accent?'selected':''}" style="background:${x}" data-value="${x}" aria-label="Farbe ${x}"></button>`).join('')}</div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-colors]').onclick=e=>{const b=e.target.closest('[data-value]');if(!b)return;draft.accent=b.dataset.value;root.querySelectorAll('[data-value]').forEach(x=>x.classList.toggle('selected',x===b));};
      root.querySelector('[data-back]').onclick=stepSymbol; root.querySelector('[data-next]').onclick=stepType;
    };

    const stepType = () => {
      const types=[['solo','🧳','Alleinreise','Zeit für dich und deine eigenen Entdeckungen.'],['couple','💞','Paarreise','Gemeinsame Zeit und Erinnerungen zu zweit.'],['family','👨‍👩‍👧','Familienreise','Pläne, Bedürfnisse und Momente für Groß und Klein.'],['group','👥','Gruppenreise','Gemeinsam mit Freunden oder mehreren Mitreisenden.']];
      const root=setStage(3,`<div class="po-slide po-centered"><div><span class="po-kicker">👥 Neue Reise · 5 von ${total}</span><h1 class="po-title small">Wer geht auf diese Reise?</h1><p class="po-copy">Damit können wir Hinweise und Funktionen später besser auf eure Reise abstimmen.</p></div><div class="po-form">${progress(5)}<div class="po-option-grid" data-options>${types.map(([v,i,t,d])=>`<button type="button" class="po-option ${v===draft.tripType?'selected':''}" data-value="${v}"><span class="po-option-icon">${i}</span>${t}<small style="display:block;margin-top:6px;font-weight:500;color:#7b8993;line-height:1.35">${d}</small></button>`).join('')}</div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-options]').onclick=e=>{const b=e.target.closest('[data-value]');if(!b)return;draft.tripType=b.dataset.value;root.querySelectorAll('[data-value]').forEach(x=>x.classList.toggle('selected',x===b));};
      root.querySelector('[data-back]').onclick=stepColor; root.querySelector('[data-next]').onclick=stepDates;
    };

    const stepDates = () => {
      const root=setStage(3,`<div class="po-slide po-centered"><div><span class="po-kicker">📅 Neue Reise · 6 von ${total}</span><h1 class="po-title small">Wann soll es losgehen?</h1><p class="po-copy">Die Daten sind optional. So kann eure Reise auch erst einmal nur eine schöne Idee bleiben.</p></div><div class="po-form">${progress(6)}<div class="po-date-grid"><div class="po-field"><label>Startdatum</label><input type="date" data-start value="${safe(draft.startDate)}"></div><div class="po-field"><label>Enddatum</label><input type="date" data-end value="${safe(draft.endDate)}"></div></div><div class="po-error"></div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-next>Weiter</button></div></div></div>`);
      root.querySelector('[data-back]').onclick=stepType;
      root.querySelector('[data-next]').onclick=()=>{const a=root.querySelector('[data-start]').value,b=root.querySelector('[data-end]').value;if(a&&b&&b<a)return showError('Das Enddatum muss nach dem Startdatum liegen.');draft.startDate=a;draft.endDate=b;stepOwner();};
    };

    const stepOwner = () => {
      const typeLabel={solo:'Alleinreise',couple:'Paarreise',family:'Familienreise',group:'Gruppenreise'}[draft.tripType]||'Reise';
      const dateText=draft.startDate ? `${new Date(draft.startDate+'T12:00:00').toLocaleDateString('de-DE')}${draft.endDate?' – '+new Date(draft.endDate+'T12:00:00').toLocaleDateString('de-DE'):''}` : 'Zeitraum noch offen';
      const root=setStage(3,`<div class="po-slide po-centered"><div><span class="po-kicker">♡ Neue Reise · 7 von ${total}</span><h1 class="po-title small">Alles bereit für eure Geschichte.</h1><p class="po-copy">Prüfe kurz die Angaben und lege fest, unter welchem Namen du als Besitzer dieser Reise erscheinst.</p></div><div class="po-form">${progress(7)}<div class="po-create-summary"><div class="po-create-symbol" style="background:${draft.accent}">${draft.symbol}</div><div><div class="po-summary-name">${safe(draft.tripName)}</div><div class="po-summary-meta">📍 ${safe(draft.destination)}<br>${typeLabel} · ${dateText}</div></div></div><div class="po-field"><label>Dein Name in dieser Reise</label><input data-name value="${safe(draft.memberName)}" autocomplete="name"></div><div class="po-error"></div><div class="po-actions"><button class="po-btn secondary" data-back>Zurück</button><button class="po-btn primary" data-go>Reise erstellen</button></div></div></div>`);
      root.querySelector('[data-back]').onclick=stepDates;
      root.querySelector('[data-go]').onclick=async event=>{
        const memberName=cleanName(root.querySelector('[data-name]').value);if(!memberName)return showError('Bitte gib deinen Namen ein.');draft.memberName=memberName;loading(event.currentTarget,true,'Reise wird erstellt …');
        try{
          const joinCode=randomCode();
          const tripId=await rpcCreate(client,draft.tripName,joinCode,memberName);
          const profile={tripId,tripName:draft.tripName,destination:draft.destination,symbol:draft.symbol,accent:draft.accent,tripType:draft.tripType,startDate:draft.startDate,endDate:draft.endDate,memberName,role:'owner',mode:draft.tripType==='solo'?'solo':'shared',joinCode,createdAt:new Date().toISOString()};
          store(profile);
          if(profile.mode==='shared') invite(profile,resolve); else success(profile,resolve);
        }catch(error){showError(error);loading(event.currentTarget,false);}
      };
    };
  ...
  function joinFromInvite(client, resolve, joinCode) {
    const root = setStage(3, `
      <div class="po-slide po-centered">
        <div><span class="po-kicker">❤️ Einladung angenommen</span><h1 class="po-title small">Wie heißt du?</h1><p class="po-copy">Du wurdest zu einer gemeinsamen Reise eingeladen. Trage nur noch deinen Namen ein, damit Fotos, Erinnerungen und Einträge später richtig zugeordnet werden.</p></div>
        <div class="po-form">
  ...
  function normalizeTrip(row) {
    return {
      ...
      tripName: row.trip_name || row.tripName || 'Unsere neue Reise',
      ...
      isOwner: Boolean(row.is_owner || row.isOwner || row.role === 'owner'),
      cloud: Boolean(row.trip_id || row.cloud),
      destination: row.destination || '',
      symbol: row.symbol || '❤️',
      accent: row.accent || '#e76f91',
      tripType: row.trip_type || row.tripType || 'couple',
  ...
  async function loadAvailableTrips(client) {
    ...
    try {
      ...
      for (const row of result.data || []) {
        ...
        if (trip.tripId) {
          const local = map.get(trip.tripId) || {};
          map.set(trip.tripId, {
            ...local, ...trip, cloud: true,
            destination: trip.destination || local.destination || '',
            symbol: trip.symbol || local.symbol || '❤️',
            accent: trip.accent || local.accent || '#e76f91',
            tripType: trip.tripType || local.tripType || 'couple',
            startDate: trip.startDate || local.startDate || '',
            endDate: trip.endDate || local.endDate || ''
          });
        }
      }
  ...
  function activateTrip(profile, resolve) {
    ...
    hide();
    resolve(clean);
    document.dispatchEvent(new CustomEvent('reisezeit:trip-selected', { detail: clean }));
  }

  function formatCreated(value) {
    if (!value) return 'Erstellungsdatum unbekannt';
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return 'Erstellungsdatum unbekannt';
    return `Erstellt am ${date.toLocaleDateString('de-DE', { day:'2-digit', month:'2-digit', year:'numeric' })}`;
  }

  async function deleteChooserTrip(client, trip) {
    const owner = trip.isOwner === true || trip.role === 'owner' || trip.role === 'admin';
    if (!owner) {
      alert('Nur der Reisebesitzer kann diese Reise löschen.');
      return false;
    }
    if (!confirm(`„${trip.tripName}“ wirklich löschen?`)) return false;
    if (trip.cloud) {
      const typed = prompt('Zur Bestätigung bitte LÖSCHEN eingeben:');
      if (typed !== 'LÖSCHEN') return false;
      const result = await client.rpc('paris_delete_trip', { p_trip_id: trip.tripId, p_confirmation: typed });
      if (result.error) throw result.error;
    }
    const next = registry().filter(item => item?.tripId !== trip.tripId);
    localStorage.setItem(REGISTRY_KEY, JSON.stringify(next));
    if (saved()?.tripId === trip.tripId) {
  ...
  async function tripChooser(client, resolve) {
    const root = setStage(2, `
      <div class="po-slide po-centered po-trip-chooser">
        <div class="po-chooser-topbar">
          <button class="po-icon-action" data-profile aria-label="Profil öffnen">👤 <span>Profil</span></button>
          <button class="po-icon-action danger" data-logout aria-label="Abmelden">↪ <span>Abmelden</span></button>
        </div>
        <div class="po-chooser-intro"><span class="po-kicker">❤️ Willkommen zurück</span><h1 class="po-title small">Welche Reise möchtest du öffnen?</h1><p class="po-copy">Wähle eine vorhandene Reise aus, erstelle eine neue oder verbinde dich per Einladungscode.</p></div>
        <div><div class="po-trip-list" data-list><div class="po-trip-empty">Cloud-Reisen werden gesucht …</div></div><div class="po-status" data-status></div><div class="po-action-stack"><button class="po-btn primary" data-new>Neue Reise erstellen</button><button class="po-btn secondary" data-join>Mit Code beitreten</button><button class="po-btn secondary" data-sync>Cloud neu synchronisieren</button></div></div>
      </div>`);
    ...
    const renderList = async () => {
      ...
      list.innerHTML = '';
      if (!trips.length) list.innerHTML = '<div class="po-trip-empty"><strong>Noch keine Reise gefunden.</strong><br>Du kannst eine neue Reise erstellen oder einer bestehenden Reise beitreten.</div>';
      for (const trip of trips) {
        const item = document.createElement('div');
        item.className = 'po-trip-option';
        const owner = trip.isOwner === true || trip.role === 'owner' || trip.role === 'admin';
        item.innerHTML = `<div class="po-trip-copy"><strong>${trip.symbol || '❤️'} ${String(trip.tripName).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}</strong>${trip.destination ? `<small>📍 ${String(trip.destination).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}</small>` : ''}<small>${owner ? 'Reisebesitzer' : 'Mitreisend'} · ${trip.memberCount || 1} Teilnehmer${trip.joinCode ? ' · ' + trip.joinCode : ''}${trip.cloud ? ' · ☁ Cloud' : ' · nur lokal bekannt'}</small><small class="po-trip-created">${formatCreated(trip.createdAt)}</small></div><div class="po-trip-actions"><button class="po-btn primary" data-open>Öffnen</button>${owner ? '<button class="po-btn danger compact" data-delete>Reise löschen</button>' : ''}</div>`;
        item.querySelector('[data-open]').onclick = event => {
          event.currentTarget.disabled = true;
          event.currentTarget.textContent = 'Wird geöffnet …';
          activateTrip(trip, resolve);
        };
        item.querySelector('[data-delete]')?.addEventListener('click', async event => {
          const button = event.currentTarget;
          button.disabled = true;
          try {
            if (await deleteChooserTrip(client, trip)) await renderList();
            else button.disabled = false;
          } catch (error) {
            alert(error?.message || 'Die Reise konnte nicht gelöscht werden.');
    ...
    root.querySelector('[data-profile]').onclick = () => {
      hide();
      window.ParisProfileCenter?.open?.('profile');
    };
    root.querySelector('[data-logout]').onclick = async () => {
      if (!confirm('Auf diesem Gerät wirklich abmelden?')) return;
      try {
        hide();
        await window.ParisAuth?.signOut?.();
      } catch (error) {
        alert(error?.message || 'Abmelden fehlgeschlagen.');
      }
    };
  ...
  window.ParisOnboarding = {
    ...
    showInvite(profileOverride = null) {
      const profile = profileOverride?.tripId ? profileOverride : saved();
