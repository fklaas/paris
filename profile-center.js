/* Reisezeit v13: globale aktive Reise, Reiseakzent, Logout, Owner-Recovery, Invite-Toplayer */
(() => {
  'use strict';
...
@media(max-width:390px){.pc-nav{grid-template-columns:1fr}.pc-side{padding-bottom:12px}.pc-main{padding-left:15px;padding-right:15px}.pc-head{display:block}.pc-head>.pc-btn{margin-top:12px}.pc-actions{grid-template-columns:1fr 1fr}}
  ...
  function openInviteAboveProfile(profile,restoreTab='trips'){const o=shell(),po=document.getElementById('parisOnboarding');o.hidden=true;document.documentElement.classList.remove('pc-open');let obs=null,done=false;const restore=()=>{if(done)return;done=true;obs?.disconnect();setTimeout(()=>window.ParisProfileCenter?.open?.(restoreTab),40)};if(po){obs=new MutationObserver(()=>{if(po.hidden)restore()});obs.observe(po,{attributes:true,attributeFilter:['hidden']})}window.ParisOnboarding?.showInvite?.(profile);setTimeout(()=>{if(po?.hidden)restore()},180)}
  ...
  function normalizeCloudTrip(row){
    ...
    return {tripId,tripName:row.trip_name||'Paris · Unser erster Hochzeitstag',joinCode:row.join_code||'',memberName:row.member_name||current()?.memberName||'Mitreisend',role:owner?'owner':(row.member_role||'member'),isOwner:owner,mode:'shared',createdAt:row.created_at,memberCount,stats:{photos:Number(row.photos||0),moments:Number(row.moments||0),expenses:Number(row.expenses||0),closures:Number(row.closures||0),notes:Number(row.notes||0),total:Number(row.total_content||0)},cloud:true};
  }
  async function cloudTrips(){
    const client=window.ParisCloud?.client;if(!client)return null;
    try{await window.ParisCloud.ready;let r=await client.rpc('paris_list_my_trips');if(r.error)throw r.error;const rows=r.data||[];for(const row of rows){if(!row.is_owner&&Number(row.member_count||0)===1){try{await client.rpc('paris_claim_unowned_trip',{p_trip_id:row.trip_id})}catch{}}}if(rows.some(row=>!row.is_owner&&Number(row.member_count||0)===1)){const rr=await client.rpc('paris_list_my_trips');if(!rr.error)r=rr}return (r.data||[]).map(normalizeCloudTrip)}catch(error){console.warn('Cloud-Reiseübersicht:',error.message);return null}
  }
