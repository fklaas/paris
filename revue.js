(()=>{
 ...
 async function build(){
  ...
  add(`<div class="scene-content"><div class="scene-kicker">Fin</div><h2>Danke für diese wunderschöne Reise.</h2><p>Paris 2026 · Unser erster Hochzeitstag · Für immer ein Teil unserer Geschichte.</p><div class="revue-final-actions"><button class="revue-final-primary" id="openBookFromRevue" type="button">📖 Reisebuch öffnen</button><a class="revue-final-secondary" href="index.html#reise-revue">↻ Zurück zur Reise-Revue</a></div></div>`,cover);
  render();
 }
 function render(){stage.innerHTML=scenes.map((s,i)=>`<section class="scene${i===0?' active':''}" data-i="${i}">${s.bg?`<div class="scene-bg" style="background-image:url('${s.bg}')"></div>`:''}${s.html}</section>`).join('');stage.addEventListener('click',e=>{if(e.target.closest('#openBookFromRevue')){sessionStorage.setItem('paris-open-book-from-revue','1');location.assign('index.html#reise-revue')}});update()}
